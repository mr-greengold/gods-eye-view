import { createTransitHistory } from './transitHistory.js';
/**
 ...
 */
export function transitProxy({ fetchImpl = fetch } = {}) {
  ...
  function install(middlewares) {
    middlewares.use('/api/transit', async (req, res) => {
      ...
      if (!inFlight.has(feed.id) && !admit(feed.id)) {
        answerWithoutUpstream('rate-limited', ttlSeconds);
        return;
      }
